...
app.get('/api/generate', async (req, res) => {
    try {
        ...
        // model=flux = Fastest model on Pollinations
        // nofeed=true = Skip feed generation (faster)
        // nologo=true = No watermark
        const imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?model=flux&width=${width}&height=${height}&nologo=true&nofeed=true&seed=${seed}`;
        
        console.log('🎨 Generating (' + width + 'x' + height + '):', prompt.substring(0, 50) + '...');
        const startTime = Date.now();
        
        const response = await fetchWithRetry(imageUrl, 60000, 2);

        // Set headers — no caching on our side too
        ...
        console.log('✅ Image generated in ' + timeTaken + 's!');
